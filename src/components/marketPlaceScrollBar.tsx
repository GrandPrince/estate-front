...
const MyComponent: React.FC<MyComponentProps> = ({ mockData }) => {
    ...
    return (
        <Shss speed={1}>
            <section className='mx-6 md:mx-16 overflow-auto'>
                <h1 className='text-white text-[22px] md:text-[42px] my-8 font-sans max-w-ShssItem'>Explore the functions of the marketplace</h1>
            </section>

            <ShssWrapper >
                {mockData.map((item, index) => (
                    <ShssSlide key={index} >
                        <div className={`w-ShssItem md:w-[646px] h-[345px] md:h-auto flex flex-row hero-border py-3 px-2 md:p-8 rounded-[30px] bg-landingBack border-t-2 border-l-2 border-iPhoneCardBorder mx-4 ${index === mockData.length - 1 ? "mr-6 md:mr-16" : ""} ${index === 0 ? "ml-5 md:ml-8" : ""}`}>
                            <Image
